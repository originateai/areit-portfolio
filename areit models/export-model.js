 ...
 * reit_model_forecasts, reit_model_outputs, reit_assets.
 *
 * Usage:   node export-model.js DXI ./DXI_model.xlsx [model_version]
 * Env:     SUPABASE_URL, SUPABASE_SERVICE_ROLE   (service role - server only)
 * Deps:    npm i xlsx @supabase/supabase-js
 *
 * IMPORTANT: the .xlsx must contain cached values (open/save in Excel, or it
 * was produced by the recalc step). SheetJS reads cell.v (the cached value).
 * ===================================================================== */
const XLSX = require('xlsx');
const { createClient } = require('@supabase/supabase-js');

const [,, TICKER, PATH, VERSION_ARG] = process.argv;
if (!TICKER || !PATH) { console.error('usage: node export-model.js TICKER path.xlsx [version]'); process.exit(1); }
const MODEL_VERSION = parseInt(VERSION_ARG || '1', 10);

const SUPABASE_URL = process.env.SUPABASE_URL;
const SERVICE_ROLE = process.env.SUPABASE_SERVICE_ROLE;
if (!SUPABASE_URL || !SERVICE_ROLE) { console.error('set SUPABASE_URL and SUPABASE_SERVICE_ROLE'); process.exit(1); }
...
const COLS = { FY24A:'C', FY25A:'D', FY26E:'E', FY27E:'F', FY28E:'G', FY29E:'H', FY30E:'I' };
...
const row  = (sheet, r, cols) => cols.map(col => cell(sheet, col + r));
...
(async () => {
  ...
  console.log(`Exporting ${TICKER} v${MODEL_VERSION} from ${PATH}`);

  // flip prior versions off
  await sb.from('reit_models').update({ is_current:false }).eq('ticker', TICKER);
  ...
  await up('reit_models', [{
    ...
    price_at_build: cell('Control','E5') ?? cell('Valuation','E96'),
  ...
  const ws = wb.Sheets['Asset Register'];
  const assets = [];
  if (ws) {
    for (let r=7; r<200; r++) {
