...
// 1. Fetches US overnight data (S&P, Nasdaq, VIX, AUD)
// 2. Fetches bond market data from FRED API
// 3. Generates composite signal score
// 4. Selects today's ASX trade candidates
// 5. Checks REIT yield triggers
// 6. Sends morning briefing email via Resend
...
function scoreSignal({ sp500Change, nasdaqChange, vix, yieldCurve, audChange, realYield }) {
  ...
  // VIX — ±2 points
  if      (vix < 15) { score += 2; reasons.push('VIX low — complacency'); }
  else if (vix < 20) { score += 1; reasons.push('VIX normal'); }
  else if (vix < 25) { score += 0; }
  else if (vix < 35) { score -= 1; reasons.push('VIX elevated — fear'); }
  else               { score -= 2; reasons.push('VIX extreme — panic'); }

  // Yield curve — ±1 point
  if      (yieldCurve >  0.005) { score += 1; reasons.push('Curve steep — growth'); }
  else if (yieldCurve < -0.002) { score -= 1; reasons.push('Curve inverted — caution'); }
...
function buildEmail(market, scored, trades, reits) {
  ...
  const tradesHtml = trades.length === 0
    ? `<tr><td colspan="6" style="padding:12px;color:#6b6660;text-align:center">No trades today — signal too weak or no setups passing all three filters (yield trigger + below 20DMA + RSI &lt; 65)</td></tr>`
    : trades.map(t => `
        <tr>
          <td class="mono" style="font-weight:600;color:#1a5f6e">${t.ticker}</td>
          <td>${t.name} <span style="font-size:10px;color:#6b6660">${t.sector}</span></td>
  ...
  const reitsHtml = reits.map(r => `
    <tr style="background:${r.fired ? '#f0f8f0' : r.close ? '#fffbf0' : 'white'}">
      ...
      <td class="mono ${r.yield_ >= 0.08 ? 'positive' : r.yield_ >= 0.07 ? 'teal' : ''}" style="font-weight:${r.yield_ >= 0.07 ? 600 : 400}">${pctRaw(r.yield_, 1)}</td>
...
<body><div class="wrap">
  ...
  <div class="section">
    ...
    ${vixAlert ? `<div style="background:#fdf8ee;border:1px solid #c8c2b4;border-left:3px solid #b8943f;padding:10px 14px;font-size:12px;color:#7a5500;font-weight:600">⚡ VIX ${vix.toFixed(1)} — ELEVATED above ${VIX_TRIGGER}. Double monthly REIT deployment this month ($24,000 instead of $12,000).</div>` : ''}
    ${firedREITs.length ? `<div style="background:#eef6ee;border:1px solid #c8c2b4;border-left:3px solid #2d5a2d;padding:10px 14px;font-size:12px;color:#2d5a2d;font-weight:600;margin-top:8px">🟢 REIT TRIGGER FIRED — ${firedREITs.map(r => r.ticker).join(', ')} at 8%+ yield. Log into IG and buy.</div>` : ''}
  </div>
  ...
  <div class="section" style="border-top:none">
    ...
    <div class="metric-row">
      ...
      <div class="metric"><div class="ml">AUD change</div><div class="mv ${colorClass(audChange)}">${pct(audChange)}</div></div>
    </div>
  </div>
  ...
  <div class="section" style="border-top:none">
    ...
    <div class="metric-row">
      ...
      <div class="metric"><div class="ml">vs Risk-free</div><div class="mv teal">+${Math.round((0.07 - BOND_YIELD) * 10000)}bps</div></div>
    </div>
    <p style="font-size:11px;color:#6b6660;margin-top:8px">
      ${yieldCurve > 0.005  ? '📈 Curve steepening — growth signal. Bullish for banks and resources.' :
        yieldCurve < -0.002 ? '⚠️ Curve inverted — recession risk. Favour defensives.' :
        '→ Curve flat — neutral. Trade on equity signal.'}
    </p>
  </div>
  ...
  <div class="section" style="border-top:none">
    ...
    <p style="font-size:11px;color:#6b6660;margin-top:8px">⏱ Execute at 10:00am AEST · Exit by 4:00pm if not at target · Never hold overnight · ⚡ = VIX boost, double size</p>
  </div>
  ...
  <div class="footer">
    Not financial advice · Paper trading mode active · Prices from Yahoo Finance · Bonds from FRED API<br>
    Dashboard: check your Netlify URL · Unsubscribe: disable morning-scan function in Netlify
  </div>
...
export const handler = async () => {
  ...
  try {
    ...
    // 2. Fetch FRED macro data in parallel
    const [realYield, breakeven, igSpread] = await Promise.all([
      fetchFRED('DFII10'),    // 10yr TIPS real yield
      fetchFRED('T10YIE'),    // 10yr breakeven inflation
      fetchFRED('BAMLC0A0CM') // IG credit spread
    ]);

    // 3. Build market data object
    const us10yr    = us10yrData?.price ? us10yrData.price / 100 : 0.0431;
    const us2yr     = 0.0474; // approximate — update via FRED if needed
    const aus10yr   = 0.0507;
    const vix       = vixData?.price    || 18;
    const aud       = audData?.price    || 0.648;
    const audChange = audData?.change   || 0;
    ...
    console.log(`Signal: ${scored.signal} (${scored.score}/10)`);

    // 5. Select trades
    const trades = await selectTrades(scored.signal, vix, market.sp500Change, db);
    console.log(`Trades selected: ${trades.length}`);

    // 6. Check REIT triggers
    const reits = await checkREITs();
    const fired = reits.filter(r => r.fired);
    console.log(`REIT triggers fired: ${fired.length}`);

    // 7. Save morning signal to Supabase
    ...
    // 8. Save paper trades to Supabase
    if (trades.length > 0) {
    ...
    // 9. Save REIT alerts to Supabase
    if (fired.length > 0) {
      await db.from('alerts').insert(fired.map(r => ({
        alert_type: 'yield_trigger',
        ticker:     r.ticker,
        message:    `${r.ticker} YIELD TRIGGER — $${r.price.toFixed(3)} — ${(r.yield_*100).toFixed(1)}% yield — BUY`,
    ...
    console.log('Morning briefing sent to James.storey@outlook.com.au');
    ...
    try {
      await sendEmail(
        ...
        `<p style="font-family:sans-serif">Morning scan failed on ${today}.<br><br>Error: ${err.message}<br><br>Check Netlify function logs.</p>`
      );
    } catch (e) { /* silent */ }
